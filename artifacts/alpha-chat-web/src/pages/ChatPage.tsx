...
export default function ChatPage({ onNavigate, requestedConvId, onConvOpened }: Props) {
  ...
  return (
    <div className="chat-root">
                ...
                return filtered.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`msg-bubble ${msg.message_type === "payment_notification" ? "payment-notification-bubble" : isMine ? "mine" : "theirs"} ${voiceMeta ? "voice-bubble" : ""} ${(msg.message_type === "payment" || msg.message_type === "usda_request" || msg.message_type === "usda_send" || msg.message_type === "wallet_payment") ? "payment-bubble" : ""} ${(msg.message_type === "sticker" || msg.message_type === "animated_sticker" || (decryptedTexts.get(msg.id) ?? "").startsWith(STICKER_MARKER) || (decryptedTexts.get(msg.id) ?? "").startsWith(ANIMATED_STICKER_MARKER)) ? "sticker-bubble" : ""} ${(msg.message_type === "text" || msg.message_type === "forward") && !voiceMeta && isEmojiOnly(decryptedTexts.get(msg.id) ?? "") ? "emoji-only-bubble" : ""}`}>
                ...
                {activeConv?.type !== "group" && (
                  <>
                    ...
                    {walletBridge.status !== "unavailable" ? (
                      ...
                      >
                        <img src={`${import.meta.env.BASE_URL}logo.png`} alt="Alpha Chat" className="attach-sheet-logo-img" />
      ...
      {/* Phase G — Alpha Wallet: bottom-sheet compatto sopra la chat.
           Ogni step sta nel viewport (nessuno scroll interno necessario);
           il CTA vive nel footer strutturale — vedi ChatWalletPaySheet.css. */}
