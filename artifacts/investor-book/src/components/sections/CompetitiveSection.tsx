  ...
  return (
    <section>
      <div className="text-center mb-16 space-y-4">
        <h2 className="text-4xl md:text-5xl font-serif text-white">{dict.competitive.title}</h2>
      ...
      <div className="space-y-24">
        ...
        <div className="space-y-6">
          <h3 className="text-2xl font-serif text-white">{dict.competitive.messaging.title}</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr>
                  {dict.competitive.messaging.columns.map((col: string, i: number) => (
                    <th key={i} className="pb-4 border-b border-white/10 text-sm font-medium text-muted-foreground uppercase tracking-wider">
                      {col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-white/5">
                {dict.competitive.messaging.rows.map((row: any, i: number) => (
                  <tr key={i} className={`hover:bg-white/[0.02] transition-colors ${row.name === 'AlphaChat' ? 'bg-primary/5' : ''}`}>
                    <td className={`py-5 font-medium ${row.name === 'AlphaChat' ? 'text-primary' : 'text-white'}`}>
                      {row.name}
                    </td>
        ...
        {/* Payments Table */}
        <div className="space-y-6">
          <h3 className="text-2xl font-serif text-white">{dict.competitive.payments.title}</h3>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr>
                  {dict.competitive.payments.columns.map((col: string, i: number) => (
                    <th key={i} className="pb-4 border-b border-white/10 text-sm font-medium text-muted-foreground uppercase tracking-wider pr-6">
                      {col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-white/5">
                {dict.competitive.payments.rows.map((row: any, i: number) => (
                  <tr key={i} className={`hover:bg-white/[0.02] transition-colors ${i === 0 ? 'bg-primary/5' : ''}`}>
                    <td className={`py-5 font-medium pr-6 ${i === 0 ? 'text-primary' : 'text-white'}`}>{row.name}</td>
        ...
        {/* Stablecoins Table */}
        <div className="space-y-6">
          <h3 className="text-2xl font-serif text-white">{dict.competitive.stablecoins.title}</h3>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <tbody className="divide-y divide-white/5">
                {dict.competitive.stablecoins.rows.map((row: any, i: number) => (
                  <tr key={i} className={`hover:bg-white/[0.02] transition-colors ${row.name === 'USDA' ? 'bg-primary/5' : ''}`}>
                    <td className={`py-5 font-medium pr-6 ${row.name === 'USDA' ? 'text-primary' : 'text-white'}`}>{row.name}</td>
