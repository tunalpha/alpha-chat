      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 py-24 md:py-32 flex flex-col md:flex-row items-center gap-12 md:gap-20">
          ...
          <div className="relative">
            ...
            <div className="relative w-[260px] md:w-[300px] bg-[#0b0918] border-2 border-white/10 rounded-[3rem] shadow-2xl overflow-hidden" style={{ aspectRatio: '9/19.5' }}>
              ...
              <div className="px-5 pt-1 pb-1">
                <div className="text-white/40 text-[8px] uppercase tracking-widest">Alpha Wallet · BIP-39/44</div>
              </div>
              {/* Balance card */}
              <div className="mx-3 rounded-2xl bg-gradient-to-br from-violet-600/30 to-violet-900/30 border border-violet-500/30 p-4 mb-2">
                <div className="text-white/50 text-[9px] mb-0.5">{isIT ? 'Saldo totale' : 'Total Balance'}</div>
                ...
                <div className="mt-2 flex gap-1.5 flex-wrap">
                  {[
                    ...
                    <span key={i} className="text-[8px] bg-white/8 border border-white/12 rounded-full px-1.5 py-0.5 text-white/50">
                      {c.icon} {c.label}
                    </span>
                  ))}
                </div>
              ...
              {/* History */}
              <div className="px-3">
                <div className="text-[9px] text-white/30 uppercase tracking-widest mb-1.5">{isIT ? 'Recenti' : 'Recent'}</div>
                <div className="flex flex-col gap-1">
                  {(d.history || []).map((item: any, i: number) => (
                    <div key={i} className="flex justify-between items-center py-1.5 border-b border-white/5">
                      <div>
                        <div className="text-[8px] text-white/65 leading-tight">{item.label}</div>
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="flex flex-wrap gap-8">
            {[
              { label: isIT ? 'Reti' : 'Networks', value: '4' },
              { label: isIT ? 'Standard' : 'Standard', value: 'BIP-39/44' },
              { label: isIT ? 'Custodia' : 'Custody', value: isIT ? 'Self-custodial' : 'Self-custodial' },
              { label: isIT ? 'Chiavi' : 'Keys', value: isIT ? 'Mai trasmesse' : 'Never sent' },
            ].map((s, i) => (
              <div key={i}>
