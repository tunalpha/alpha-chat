...
export default function SwotSection({ dict }: { dict: any }) {
  const quadrants = [
    ...
    { key: 'w', color: 'border-white/10 bg-white/[0.02]' },
    { key: 'o', color: 'border-white/10 bg-white/[0.02]' },
    { key: 't', color: 'border-white/10 bg-white/[0.02]' },
  ];

  return (
    <section>
      <div className="text-center mb-16">
        <h2 className="text-4xl md:text-5xl font-serif text-white">{dict.swot.title}</h2>
      </div>
        ...
        {quadrants.map((quad, i) => {
          ...
          return (
            <div key={i} className={`p-8 border rounded-2xl ${quad.color}`}>
              <h3 className="text-xl font-serif text-white mb-6 uppercase tracking-widest opacity-90">{data.title}</h3>
