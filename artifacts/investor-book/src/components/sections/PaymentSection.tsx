    ...
    <section>
      <div className="p-8 md:p-12 border border-primary/20 bg-primary/[0.02] rounded-3xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-4xl md:text-5xl font-serif text-foreground mb-4">{dict.paymentLayer.title}</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-8">
            {dict.paymentLayer.features.map((feat: any, i: number) => (
              <div key={i} className="bg-background/50 border border-border/50 p-6 rounded-2xl backdrop-blur-sm">
                <div className="w-2 h-2 rounded-full bg-primary mb-4 shadow-[0_0_10px_rgba(168,85,247,0.5)]" />
                <h3 className="text-foreground font-medium mb-2">{feat.title}</h3>
